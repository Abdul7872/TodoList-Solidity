...
export default function Home() {
  ...
  const connectWallet = async () => {
    try{
      ...
      if(!ethereum){
        console.log('Metamask not detected... Pls connect Metamask!');
        alert('Metamask not detected... Pls connect Metamask!');
        return;
      }
  ...
  return (
    <div className='bg-[#97b5fe] h-screen w-screen flex justify-center py-6'>
      { !isUserLoggedIn ? <ConnectWalletButton connectWallet={connectWallet} /> :
        correctNetwork ? <TodoList {...{input,setInput,addTask,tasks,deleteTask}} /> : <WrongNetworkMessage />}
    </div>
  )
}
